...
* @author  Lei Lu
************************************************************************/
bool MaterialManager::InsertNewMaterial(int theID,
                                        double  theDensity,
                                        QString theName,
                                        QString theMatCard)
{
...
* @author  Lei Lu
************************************************************************/
bool MaterialManager::DeleteMaterial(int index)
{
    map<int,Material *>::iterator iter;
...
* @author  Lei Lu
************************************************************************/
bool MaterialManager::UpdateMaterial(int    index,
                                                 int    theID,
                                                 double theDensity,
                                                 QString    theName,
                                                 QString    theMatCard)
...
* @author  Lei Lu
************************************************************************/
bool MaterialManager::InsertMatGroup(QString &GroupName, int &MateriaID)
{
    map<int,Material*>::iterator it;
    it = m_mapMaterial.find(MateriaID);
    if (it != m_mapMaterial.end())
    {
        Material *pMat = it->second;
        MaterialGroup * pGroup = new MaterialGroup(0,100,GroupName,*pMat);
...
* @author  Lei Lu
************************************************************************/
bool MaterialManager::GenSolidGroupMap(int iSolidIDBegin, int iSolidIDEnd,
                                                   QString strMatGroup)
{
    if (0 == iSolidIDEnd)
    {
...
* @date 04/11/2013
* @author  Lei Lu
* @modify 02/08/2018 repaired the problem of last group with 1 solid
* can not be added into the xml file
************************************************************************/
void MaterialManager::SaveSolidGroupMap(QDomElement &prntItem, QDomDocument &domDoc)
{
    ...
    for (iter = m_mapSolidGroup.begin(); iter != m_mapSolidGroup.end(); ++iter)
    {
        int iSolidID = iter->first;
        QString strNextGroup = iter->second;

        if (strNextGroup != strGroup)
        {
            iGroup ++;
            iEnd = iSolidID-1;
        }
    ...
    // Process the last element in the map
    map<int,QString>::reverse_iterator rev_iter;
    rev_iter = m_mapSolidGroup.rbegin();

    QString strLastGroup = rev_iter->second;
    int iSolidID = rev_iter->first;
    if(strLastGroup != strGroup)
    {
        iGroup ++;
        iBegin = iSolidID;
        iEnd = iSolidID;
    ...
    QDomText txtBegin = domDoc.createTextNode(QString::number(iBegin));
    QDomText txtEnd = domDoc.createTextNode(QString::number(iEnd));
    QDomText txtMaterial = domDoc.createTextNode(strLastGroup);
...
* @author  Lei Lu
************************************************************************/
bool MaterialManager::LoadXMLfile(QString theFileName, bool bRdSldGrpMap)
{
...
bool MaterialManager::RemoveMaterials()
{
    map<int,Material *>::iterator iter;
    iter = m_mapMaterial.begin();
    while(iter != m_mapMaterial.end())
    ...
    m_mapGroupMaterial.clear();
    //m_mapSolidGroup.clear();   // solid group map is created when the dialogbox is launched
    m_groupInfo.clear();
...
bool MaterialManager::ClearSolidGroupMap()
{
    m_mapSolidGroup.clear();
}
