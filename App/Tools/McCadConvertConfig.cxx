...
unsigned int McCadConvertConfig::m_iInitVoidBoxNum = 1;
...
Standard_Integer McCadConvertConfig::GetSurfSequNum(TCollection_AsciiString SurfSymb)
{
...
{
    // Lei2018
    //McCadMessenger_Singleton* msgr = McCadMessenger_Singleton::Instance(); Lei2018
...
//        //codeword with parameter
            ...
            else if(iString.IsEqual("INITIALVOIDBOXNUMBER")) {
                if(!numString.IsIntegerValue() && !numString.IsRealValue())
                    MissmatchMessage(iString,numString);
                else
                    m_iInitVoidBoxNum = numString.IntegerValue();
            }
            else if(iString.IsEqual("VOIDGENERATE")) {
                if( numString.IsEqual("Yes")    || numString.IsEqual("yes") ||
                    numString.IsEqual("YES") || numString.IsEqual("1")  )
                {
