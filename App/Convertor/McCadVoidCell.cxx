...
{ 
    ...
    float fs = 100.0/m_CollidedSolidNumList.size();

    for (unsigned int i = 0; i < m_CollidedSolidNumList.size(); i++)
    {        
        msg->sig_progress_part(int(fs*(i+1)));
        ...
        for (unsigned int j = 0; j < pSolid->GetSTLFaceList().size(); j++)
        {
            ...
            if(hasSameFaces(pFace))
            {                
                continue;
            }

	    if (CalColliedTriangle(pFace->GetTriangleList()))
            {
    ...
    msg->sig_progress_part(100);
}



/** ********************************************************************
* @brief If the void box has same surfaces with the face to be checked
*        There are no collision between them.
*
*        Because of the precision problem, some sample point is not located
*        on the surface exactly. There are small distance between them.
*        But if these surface are contact with surface of void box, it will
*        bring the wrong judgement, and then cause error in void description.
*
*
...
* @author  Lei Lu
************************************************************************/
Standard_Boolean McCadVoidCell::HasSameFaces(McCadExtBndFace *& pFace)
{
    ...
    for (unsigned int i = 0; i < m_BndFaceList.size(); i++)
    {
        ...
        if(pExtFace->GetFaceType() == GeomAbs_Plane && pFace->GetFaceType() == GeomAbs_Plane)
        {
            GeomAdaptor_Surface gs_box = pExtFace->GetAdpFace();
            GeomAdaptor_Surface gs_sld = pFace->GetAdpFace();

            gp_Pln plane_box = gs_box.Plane();
            gp_Pln plane_sld = gs_sld.Plane();

            if( plane_box.Position().Direction().IsEqual(plane_sld.Position().Direction(), angle_tol))
            {
                if(plane_box.Distance(plane_sld.Location())<dis_tol)
                {
                   return Standard_True;
                }
            }

            if( plane_box.Position().Direction().IsOpposite(plane_sld.Position().Direction(), angle_tol))
            {
                if(plane_box.Distance(plane_sld.Location())<dis_tol)
                {
                    return Standard_True;
...
* @author  Lei Lu
************************************************************************/
Standard_Boolean McCadVoidCell::CalColliedTriangle(vector<McCadTriangle*> triangle_list)
{
    Standard_Boolean bInVoid = Standard_False;

    vector<McCadTriangle *>::iterator iterTri;
    for (iterTri = triangle_list.begin(); iterTri != triangle_list.end(); ++iterTri)
    {
        McCadTriangle * pTri = *iterTri;
        assert(pTri);

        // By boundary box
        Bnd_Box bbox_tri = pTri->GetBndBox();
		if (!CalColliedBox(bbox_tri))
		{
			continue;
		}

        // By vertex
        for (Standard_Integer i = 1; i <= pTri->GetVexList()->Length();i++)
        {
                gp_Pnt pnt = pTri->GetVexList()->Value(i);
         ...
         for (unsigned int i = 0; i < m_BndFaceList.size(); i++)
         {
                ...
                {
                    BRepAlgoAPI_Section section(*pExtFace, *pTri);
		 ...
		 /* for (unsigned int i = 0; i < m_BndFaceList.size(); i++)
		  {
			  McCadExtBndFace * pExtFace = m_BndFaceList[i];
			  Handle(Geom_Surface) S2 = BRep_Tool::Surface(*pExtFace, loc);

			  GeomAPI_IntSS Intersector(S1, S2, 1.0e-5);
...
Standard_Boolean McCadVoidCell::CalColliedPoints(Handle_TColgp_HSequenceOfPnt point_list)
{
    ...
    for (Standard_Integer i = 1; i <= point_list->Length(); i++)
    {
...
{     
    ...
    if(theFace->GetSameFaces().size()!=0)
    {
        ...
        for (unsigned int i = 0 ; i < theFace->GetSameFaces().size(); i++)
        {
...
Standard_Boolean McCadVoidCell::IsVertexInSolid(McCadConvexSolid *& pSolid)
{
    ...
    for (Standard_Integer i = 1; i <= m_VertexList->Length(); i++)
    {
...
TCollection_AsciiString McCadVoidCell::GetExpression()
{
    ...
    for (unsigned int i = 0; i < m_CollisionList.size(); i++)
    {
        McCadVoidCollision *pCollision = m_CollisionList.at(i);
        m_szExpression += "(";
        for (unsigned int j = 0; j < pCollision->GetFaceNumList().size(); j++)
        {
        ...
        if ( pCollision->GetAstFaceNumList().size() > 1)
        {
            ...
            for (unsigned int k = 0; k < pCollision->GetAstFaceNumList().size(); k++)
            {
...
{        
    ...
    if (this->GetExpression().Length() > McCadConvertConfig::GetMaxDiscLength())
    {
        ...
        //msg->sig_progress_part(0);
        pVoidA->CalColliedFaces(pData);
        //msg->sig_progress_part(50);
