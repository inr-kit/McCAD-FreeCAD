...
void McCadTripoliWriter::PrintCellDesc(Standard_OStream& theStream)
{
    ...
    for (unsigned int i = 0; i < (pData->m_SolidList).size(); i++)
    {
        ...
        for(int j = 0; j < iNumEpn; j++)
        {
            ...
            if (j < iNumEpn-1 )
            {
                ...
                if(!sub_list.empty())
                {
                    ...
                    for(unsigned int iSubCell = 0; iSubCell < sub_list.size(); iSubCell++)
                    {
        ...
        if (iNumEpn > 1)
        {
            ...
            for(unsigned int m = 0; m < union_list.size(); m++)
            {
                if (m % 10 == 0 && m != 0)
                {
                    theStream<<endl;
        ...
        for( unsigned int k = 0; k < cell_list.size(); k++ )
        {
...
void McCadTripoliWriter::PrintVoidDesc(Standard_OStream& theStream)
{
    ...
    for (unsigned int i = 0; i < (pData->m_VoidCellList).size(); i++)
    {
        ...
        if (iNumEpn > 1)
        {
            ...
            for(unsigned int m = 0; m < complement_list.size(); m++)
            {
                if (m % 10 == 0 && m != 0)
                {
                    theStream<<endl;
...
void McCadTripoliWriter::PrintOutSpace(Standard_OStream& theStream)
{
    ...
    for(unsigned int m = 0; m < outer_list.size(); m++)
    {
...
void McCadTripoliWriter::PrintMatCard(Standard_OStream& theStream)const
{
    ...
    for(unsigned int i = 0; i < material_list.size(); i++)
    {
    ...
    for(int j = 0; j < iConvexSolid; j++)
    {
        ...
        for (unsigned j = 0; j < IntCellList.size(); j++)
        {
            ...
            if(UniCellList.size() > 1)
            {
                ...
                for(unsigned int k = 0; k < UniCellList.size(); k++)
                {
...
{
    vector<McCadTripoliCell *> cell_list;
    for (unsigned int i = 0; i < pVoidCell->GetCollisions().size(); i++)
    {
        McCadTripoliCell *pTriCell = new McCadTripoliCell();
        ...
        {
            McCadTripoliCell *pCldCell = new McCadTripoliCell();
            for (unsigned int j = 0; j < pCollision->GetAstFaceNumList().size(); j++)
            {
    ...
    for (unsigned int i = 0; i < (pVoidCell->GetBndFaces()).size(); i++)
    {
...
void McCadTripoliWriter::PrintSurfDesc(Standard_OStream & theStream)
{
    ...
    for (iterPos = (pData->m_GeomSurfList).begin(); iterPos != (pData->m_GeomSurfList).end(); ++iterPos)
    {
       ...
       for (unsigned int i = 0; i < surf_list.size(); i++ )
       {
           if ( i%4 == 0 && i != 0 )
           {
               theStream<<endl;
