...
#include "../Tools/McCadEvaluator.hxx"
...
McCadConvexSolid::McCadConvexSolid(const TopoDS_Solid & theSolid):TopoDS_Solid(theSolid)
{
    ...
    msg = McCadMessager_Signal::Instance();         //initiate the message signal
...
void McCadConvexSolid::GenFacesList(const TopoDS_Solid & theSolid)
{
    ...
    msg->sig_part_label(QString::fromLatin1("Generate the surface list"));
    msg->sig_progress_part(0);    // Set the progress bar to 0

    Handle_TopTools_HSequenceOfShape face_list = new TopTools_HSequenceOfShape;
    ...
    for(exp.Init(theSolid, TopAbs_FACE); exp.More(); exp.Next())
    {
        ...
        face_list->Append(theFace);
        m_iNumOfFaces++;
    }

    m_iNumOfFaces = face_list->Length();

    for (int i = 1; i <= m_iNumOfFaces; i++)
    {
        TopoDS_Face theFace = TopoDS::Face(face_list->Value(i));
        McCadExtBndFace *pExtFace = new McCadExtBndFace(theFace);   // Create the extended faces

        Bnd_Box theBB;
	BRepBndLib::Add(theFace, theBB);
	theBB.SetGap(0.0);
	Standard_Real deflection = sqrt(theBB.SquareExtent());
	deflection /= 100.0;

	pExtFace->SetDeflection(deflection);
        m_STLFaceList.push_back(pExtFace);        
        msg->sig_progress_part(int(i*100/m_iNumOfFaces));           // Move the progress bar
    }
    ...
    for( unsigned int i = 0; i < m_STLFaceList.size()-1; i++ )
    {
		...
		Bnd_Box theBB;
		BRepBndLib::Add(newGenFace, theBB);
		theBB.SetGap(0.0);
    ...
    msg->sig_progress_part(100);    // Set the progress bar to 0
...
{
    msg->sig_part_label(QString::fromLatin1("Add assistant surfaces"));
    msg->sig_progress_part(0);    // Set the progress bar to 0
    float fs = 100.0/m_STLFaceList.size();
    ...
    if (m_DiscPntList->IsEmpty())
    {
        if (bGenVoid)
        {
            for(unsigned int i = 0; i < m_STLFaceList.size(); i++)
            {
                ...
                m_DiscPntList->Append(pExtFace->GetDiscPntList());    // Add the discrete point list of face into the one of solid
                msg->sig_progress_part(int(fs*(i+1)));
            }
        }
    }
...
* @author  Lei Lu
************************************************************************/
void McCadConvexSolid::GenTriangles(Standard_Boolean bGenVoid)
{
    /* Generate sample points of the each face */
    if (m_TriangleList.empty() && bGenVoid)
    {
        for(unsigned int i = 0; i < m_STLFaceList.size(); i++)
        {
			McCadExtBndFace *pExtFace = m_STLFaceList[i];
            pExtFace->GetTriangleList();
        }
    }
}
...
{
    msg->sig_part_label(QString::fromLatin1("Create the sample points of edges"));
    msg->sig_progress_part(0);    // Set the progress bar to 0
    float fs = 100.0/m_STLFaceList.size();
...
void McCadConvexSolid::JudgeAssistFaces(vector<McCadExtAstFace*> & theAstFaceList)
{
    ...
    for (unsigned int i = 0; i < theAstFaceList.size(); i++)
    {
        ...
        for (Standard_Integer j = 1; j <= m_EdgePntList->Length(); j++)
        {
            ...
            if (iPosPnt > 0 && iNegPnt > 0) // i.e sign changing, the assited surface can split solid
            {
               ...
               // pExtAstFace->SetSplit(Standard_True);  // LeiLu 20170809 modify
                break;
            }
        }        
    }
...
{
    for (unsigned int i = 0; i < pLeftFace->GetAstFaces().size(); i++)
    {
        McCadExtAstFace *pLeftAstFace = pLeftFace->GetAstFaces().at(i);
        for (unsigned int j = 0; j < pRightFace->GetAstFaces().size(); j++)
        {
