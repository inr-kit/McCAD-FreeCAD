                                                     ...
                                                     const McCadExtBndFace *&pFace )
{
    ...
    GeomAdaptor_Surface adpFace = pFace->GetAdpFace();
    ...
    else if( pFace->GetFaceType() == GeomAbs_Cone )
    {
        v1 = gp_Vec(P2,P1);
        v2 = gp_Vec(P3,P1);
        v3 = v2 ^ v1;
	...
	/*	v1 = gp_Vec(P2, P1);
		if (fabs(P3.X() - P1.X()) < dis_tol && fabs(P3.Y() - P1.Y()) < dis_tol && fabs(P3.Z() - P1.Z()) < dis_tol)  // ¹ý¶¥µã
		{
    ...
    else if( pFace->GetFaceType() == GeomAbs_Sphere)
    {
        P3 = (HndGeomFace->Value(UMin,VMin+((VMax - VMin)/2.0)) ).Transformed(T);
        P4 = (HndGeomFace->Value(UMax,VMin+((VMax - VMin)/2.0)) ).Transformed(T);
    ...
    (Abs(UMax - UMin) <= M_PI)? pnt_ast = pnt_center:P1;
    //pnt_ast = P1;


    /* 20170620 modified for the TBM PP model */
